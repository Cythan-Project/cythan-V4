...
fn mutated_slot_is_forgotten_after_loop() {
    ...
    let input = blk(vec![
        HirOp::Set(sid(0), 3),
        HirOp::Loop(blk(vec![HirOp::MapValue(sid(0), sid(0), crate::ir::INC_TABLE)])),
        HirOp::Copy(sid(1), sid(0)),
    ]);
    let out = specialize_to_fixpoint(input);
    assert!(
...
fn readonly_slot_keeps_domain_inside_loop() {
    ...
    let input = blk(vec![
        HirOp::Set(sid(0), 5),
        HirOp::Loop(blk(vec![
            HirOp::MapValue(sid(1), sid(1), crate::ir::INC_TABLE),
    ...
    let input = blk(vec![
        ...
        HirOp::Match(
            ...
            vec![
                ...
                (blk(vec![HirOp::MapValue(sid(3), sid(3), crate::ir::INC_TABLE)]), (1..=15).collect()),
            ],
        ),
...
fn readonly_slot_keeps_domain_across_loop_exit() {
    ...
    let input = blk(vec![
        HirOp::Set(sid(0), 7),
        HirOp::Loop(blk(vec![HirOp::MapValue(sid(1), sid(1), crate::ir::INC_TABLE), HirOp::Break])),
